<!-- source: https://json-render.dev/docs/specs -->
# Specs
...
A spec (specification) is the actual JSON describing a UI. It uses components from a catalog and can optionally follow a schema. Specs can be:
...
A basic spec using the `@json-render/react` schema with a flat structure containing a `root` key and `elements` map:
...
Specs are schema-agnostic — the JSON structure is entirely up to you. The examples use the `root` + `elements` flat tree format from the `@json-render/react` schema, which is optimized for AI generation and streaming.
