...
- `renderToStream(spec)` — render to a readable stream
...
Schemas can now declare `builtInActions` — actions that are always available at runtime and automatically injected into prompts. The React schema declares `setState`, `pushState`, and `removeState` as built-in.
...
```javascript
// Standalone mode (default) — AI outputs only JSONL
const prompt = catalog.prompt();

// Inline mode — AI outputs text + JSONL inline
